...
export default function HomeScreen({ navigation }: Props) {
  ...
  const quickActions = [
    ...
    { name: 'Pay Request', icon: require('../../assets/Pay request.png'), bgColor: '#E0F2F1', hasNotification: true, onPress: () => navigation.navigate('PaymentReport') },
  ...
  return (
    <View style={{ flex: 1, backgroundColor: '#F5F5F5' }}>
      ...
      <ScrollView style={{ flex: 1 }} contentContainerStyle={{ paddingTop: 40, paddingBottom: 100 }}>
        {/* Top Status Bar (commented out — using OS status bar instead) */}
        {false && (
          <View style={{ backgroundColor: 'white', paddingTop: 44, paddingHorizontal: 20, paddingBottom: 12, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }}>
            <View style={{ flexDirection: 'row', alignItems: 'center' }}>
              <Text style={{ fontSize: 15, fontWeight: '600', color: '#000', marginRight: 8 }}>9:41</Text>
              <Image source={require('../../assets/ESI.png')} style={{ width: 18, height: 18, resizeMode: 'contain' }} />
            </View>
        ...
        <View style={{ marginHorizontal: 16, marginTop: 20, marginBottom: 24 }}>
          ...
          {/* Top shortcut row: Contacts, Tasks, History, Payment */}
          <View style={{ flexDirection: 'row', justifyContent: 'space-between', marginBottom: 16 }}>
            {[ 
              ...
              { name: 'Payment', icon: require('../../assets/payment.png'), onPress: () => navigation.navigate('PaymentReport') },
            ].map((item, idx) => (
              <View key={`top-${idx}`} style={{ width: '23%' }}>
                <TouchableOpacity onPress={(item as any).onPress} style={{ backgroundColor: '#FFFFFF', borderRadius: 16, alignItems: 'center', justifyContent: 'center', aspectRatio: 1, position: 'relative', padding: 12, shadowColor: '#000', shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.08, shadowRadius: 8, elevation: 3 }}>
          ...
          <View style={{ flexDirection: 'row', flexWrap: 'wrap', justifyContent: 'space-between' }}>
            {quickActions.map((action, index) => (
              <View key={index} style={{ width: '23%', marginBottom: 16 }}>
                <TouchableOpacity onPress={(action as any).onPress} style={{ backgroundColor: action.bgColor, borderRadius: 16, alignItems: 'center', justifyContent: 'center', aspectRatio: 1, position: 'relative', padding: 12 }}>
